#!/usr/bin/env python3
"""
TAIRID ETDD70 response-shape test v2.
...
This v2 script fixes the v1 CSV-writing failure. Mixed inventory/download rows can contain
different fields such as error/path/status, so write_csv now builds a union of keys.
...
OUTDIR = Path("tairid_etdd70_response_shape_v2_outputs")
...
def read_url(url, timeout=240):
    req = urllib.request.Request(
        url,
        headers={
            "User-Agent": "TAIRID-ETDD70-response-shape-v2",
...
def download_zenodo_record():
    ...
    (OUTDIR / "zenodo_record_13332134.json").write_text(json.dumps(record, indent=2), encoding="utf-8")
...
def load_group_label_map(downloads):
    """
    ...
    for root in [DOWNLOAD_DIR, EXTRACT_DIR]:
        for path in root.rglob("*"):
            ...
            if "label" not in path.name.lower() and "class" not in path.name.lower() and "dys" not in path.name.lower():
                continue

            if not path.name.lower().endswith((".csv", ".tsv", ".txt", ".xlsx", ".xls")):
                continue
            ...
            if group_col is None:
                # Use last column as a fallback when file looks like a label sheet.
            ...
            for _, row in df.iterrows():
                ...
                if subj and grp:
                    label_map[subj] = grp
                    # Also store simplified numeric substring if available.
...
def aggregate_subject_task_features(feature_rows):
    ...
    return usable.groupby(group_cols, dropna=False)[numeric_cols].median().reset_index()
...
def plot_outputs(task_df, subject_df):
    ...
    if not task_df.empty and "T_pacing_proxy" in task_df.columns:
        ...
        plt.title("TAIRID ETDD70 response shape v2: mismatch across task demand")
        plt.legend()
        plt.tight_layout()
        path = OUTDIR / "etdd70_mismatch_across_task_v2.png"
        ...
        plt.title("TAIRID ETDD70 response shape v2: collapse load across task demand")
        plt.legend()
        plt.tight_layout()
        path = OUTDIR / "etdd70_collapse_load_across_task_v2.png"
        plt.savefig(path, dpi=160)
        plt.close()
        plots.append(str(path))
    ...
    if not subject_df.empty and "mismatch_slope_over_task" in subject_df.columns:
        ...
        if values:
            ...
            plt.title("TAIRID ETDD70 response shape v2: subject-level mismatch slope")
            plt.tight_layout()
            path = OUTDIR / "etdd70_subject_mismatch_slope_boxplot_v2.png"
            plt.savefig(path, dpi=160)
            plt.close()
            plots.append(str(path))
...
def main():
    print("")
    print("TAIRID ETDD70 response-shape test v2 starting.")
    ...
    write_csv(OUTDIR / "etdd70_download_ledger_v2.csv", downloads)
    write_csv(OUTDIR / "etdd70_extraction_ledger_v2.csv", extraction)

    label_map = load_group_label_map(downloads)
    label_map_rows = [{"subject_key": k, "group": v} for k, v in sorted(label_map.items())]
    write_csv(OUTDIR / "etdd70_label_map_v2.csv", label_map_rows)

    inventory, feature_rows = table_inventory_and_features(label_map)

    inventory_path = write_csv(OUTDIR / "etdd70_table_inventory_v2.csv", inventory)
    features_path = write_csv(OUTDIR / "etdd70_raw_feature_rows_v2.csv", feature_rows)

    subject_task = aggregate_subject_task_features(feature_rows)
    if not subject_task.empty:
        subject_task_path = OUTDIR / "etdd70_subject_task_features_v2.csv"
    ...
    if not task_shape_df.empty:
        task_shape_path = OUTDIR / "etdd70_tairid_task_shape_features_v2.csv"
        task_shape_df.to_csv(task_shape_path, index=False)
    else:
        task_shape_path = None

    if not subject_shape_df.empty:
        subject_shape_path = OUTDIR / "etdd70_tairid_subject_response_shape_v2.csv"
    ...
    analysis_path = OUTDIR / "etdd70_response_shape_analysis_v2.json"
    ...
    summary = {
        "test_name": "TAIRID ETDD70 response-shape test v2",
        "boundary": (
        ...
        "analysis": analysis,
        "output_files": {
            "download_ledger_csv": str(OUTDIR / "etdd70_download_ledger_v2.csv"),
            "extraction_ledger_csv": str(OUTDIR / "etdd70_extraction_ledger_v2.csv"),
            "label_map_csv": str(OUTDIR / "etdd70_label_map_v2.csv"),
    ...
    summary_path = OUTDIR / "etdd70_response_shape_v2_summary.json"
    summary_path.write_text(json.dumps(summary, indent=2), encoding="utf-8")

    with open(OUTDIR / "etdd70_response_shape_v2_summary.txt", "w", encoding="utf-8") as f:
        f.write("TAIRID ETDD70 response-shape test v2\n\n")
    ...
    print("")
    print("TAIRID ETDD70 response-shape test v2 complete.")
    print("Created:")
    print("  tairid_etdd70_response_shape_v2_outputs/etdd70_response_shape_v2_summary.json")
    print("  tairid_etdd70_response_shape_v2_outputs/etdd70_response_shape_v2_summary.txt")
    print("  tairid_etdd70_response_shape_v2_outputs/etdd70_table_inventory_v2.csv")
    print("  tairid_etdd70_response_shape_v2_outputs/etdd70_raw_feature_rows_v2.csv")
    print("  tairid_etdd70_response_shape_v2_outputs/etdd70_response_shape_analysis_v2.json")
    print("")
    print("Boundary:")
