#!/usr/bin/env python3
"""
TAIRID Table2 F160W threshold-sweep audit v1.4.

Purpose:
The v1.3 F160W uncertainty-selection audit found that the strongest remaining
SH0ES Table2 residual pressure is not the 42<->46 H0 bridge and not host identity alone.

It concentrates around the F160W faint-end / selection edge:

    f160w_top10_table2_num_7

This test asks whether that signal behaves like a true boundary edge.

It sweeps F160W thresholds:

    faint end / high F160W magnitude:
        top 5%, 10%, 15%, 20%, 25%

    bright end / low F160W magnitude:
        bottom 5%, 10%, 15%, 20%, 25%

It audits each threshold against:
    - original 47-parameter SH0ES compact ladder
    - plus host controls
    - plus uncertainty controls
    - plus measurement controls
    - plus host + measurement controls
    - plus host + row-order + measurement controls

Boundary:
This is not proof of TAIRID.
This is not H0 resolution.
This is not BAO, Planck, or a full cosmology model.
This only tests whether the Table2 F160W residual signal is shaped like a threshold boundary.
"""
...
OUTDIR = Path("tairid_table2_f160w_threshold_sweep_v1_4_outputs")
...
def fetch_url(url):
    req = urllib.request.Request(
        url,
        headers={
            "User-Agent": "TAIRID-Table2-F160W-threshold-sweep-v1-4",
            "Accept": "*/*",
        },
    )
...
def parse_table2_tex(path):
    ...
    write_csv(OUTDIR / "table2_all_rows_v1_4.csv", all_rows)
    write_csv(OUTDIR / "table2_data_rows_v1_4.csv", data_rows)
    write_csv(OUTDIR / "table2_header_rows_v1_4.csv", header_rows)
...
def map_table2_to_spine(row_rows, table2_data_rows):
    ...
    write_csv(OUTDIR / "table2_compact_host_mapped_rows_v1_4.csv", mapped)
...
def summarize_hosts(mapped_rows):
    ...
    write_csv(OUTDIR / "table2_host_summary_v1_4.csv", host_rows)
...
def numeric_feature_summary(mapped_rows):
    ...
    write_csv(OUTDIR / "table2_numeric_summary_v1_4.csv", rows)
    write_json(OUTDIR / "table2_likely_numeric_labels_v1_4.json", likely)
...
def threshold_mask_from_f160w(mapped_rows, y_length, percentile, side):
    values, meta = vector_from_mapped_numeric(mapped_rows, y_length, "table2_num_7")
    ...
    if side == "faint_top":
        threshold = float(np.percentile(spine_values, 100 - percentile))
        comparator = ">="
    elif side == "bright_bottom":
    ...
    for row in mapped_rows:
        ...
        if side == "faint_top" and values[idx] >= threshold:
            mask[idx] = 1.0

        if side == "bright_bottom" and values[idx] <= threshold:
            mask[idx] = 1.0
...
def build_designs(X, mapped_rows, host_summary, y_length):
    ...
    write_json(OUTDIR / "control_design_metadata_v1_4.json", metadata)

    return designs, metadata


def build_threshold_candidates(mapped_rows, y_length):
    candidates = []

    for side in ["faint_top", "bright_bottom"]:
        for percentile in THRESHOLDS:
            values, meta = threshold_mask_from_f160w(
                mapped_rows,
                y_length,
                percentile,
                side,
            ...
            candidates.append(
                {
                    "name": f"f160w_{side}_{percentile:02d}",
                    "values": values,
                    "kind": "f160w_threshold_mask",
                    "side": side,
                    "percentile": percentile,
    ...
    # Continuous control, for comparison.
    f160w_values, f160w_meta = vector_from_mapped_numeric(mapped_rows, y_length, "table2_num_7")

    candidates.append(
        {
            "name": "f160w_continuous_table2_num_7",
            "values": f160w_values,
            "kind": "f160w_continuous",
            "side": "continuous",
            "percentile": None,
            "threshold": None,
            "count": int(np.sum(np.abs(f160w_values) > 1.0e-12)),
            "metadata": f160w_meta,
        }
    )

    write_json(
        OUTDIR / "threshold_candidate_inventory_v1_4.json",
        [
            {
                "name": c["name"],
                "kind": c["kind"],
...
def run_audits(y, c_factor, design_fits, candidates):
    ...
    rows = sorted(
        ...
        key=lambda r: (
            r["design"],
            r.get("side") or "",
            r.get("percentile") if r.get("percentile") is not None else 999,
            -float(r.get("delta_chi2_score", 0.0)),
        ),
    )

    write_csv(OUTDIR / "threshold_audit_all_designs_v1_4.csv", rows)
...
def run_selected_permutation_controls(mapped_rows, y, c_factor, design_fits, candidates):
    ...
    selected_candidates = [
        ...
        if c["kind"] == "f160w_threshold_mask"
    ]
    ...
    write_csv(OUTDIR / "threshold_permutation_control_details_v1_4.csv", all_rows)
    write_json(OUTDIR / "threshold_permutation_control_summaries_v1_4.json", summaries)

    return all_rows, summaries


def summarize_threshold_profiles(audit_rows, permutation_summaries):
    perm_key = {
    ...
    for row in audit_rows:
        if row.get("candidate_kind") != "f160w_threshold_mask":
            continue

        key = (row["design"], row["candidate"])
        perm = perm_key.get(key, {})
    ...
    write_csv(OUTDIR / "threshold_profile_summary_v1_4.csv", rows)

    profile_by_design = {}

    for design in sorted(set(r["design"] for r in rows)):
        design_rows = [r for r in rows if r["design"] == design]
        faint_rows = [r for r in design_rows if r["side"] == "faint_top"]
        bright_rows = [r for r in design_rows if r["side"] == "bright_bottom"]

        best_faint = max(faint_rows, key=lambda r: r["delta_chi2_score"]) if faint_rows else None
        best_bright = max(bright_rows, key=lambda r: r["delta_chi2_score"]) if bright_rows else None

        faint_values = np.asarray([r["delta_chi2_score"] for r in faint_rows], dtype=float)
        bright_values = np.asarray([r["delta_chi2_score"] for r in bright_rows], dtype=float)

        profile_by_design[design] = {
            "best_faint": best_faint,
            "best_bright": best_bright,
            "faint_delta_values": [float(v) for v in faint_values],
            "bright_delta_values": [float(v) for v in bright_values],
            "faint_peak_to_median_ratio": (
                float(np.max(faint_values) / max(np.median(faint_values), EPS))
                if len(faint_values) else None
            ),
            "bright_peak_to_median_ratio": (
                float(np.max(bright_values) / max(np.median(bright_values), EPS))
                if len(bright_values) else None
            ),
            "faint_peak_minus_bright_peak": (
                float(np.max(faint_values) - np.max(bright_values))
                if len(faint_values) and len(bright_values) else None
            ),
        }

    write_json(OUTDIR / "threshold_profile_by_design_v1_4.json", profile_by_design)

    return rows, profile_by_design


def decide_status(profile_by_design):
    full_design = "plus_host_top10_row_order_measurement_controls"
    mid_design = "plus_host_top10_and_measurement_controls"
    original_design = "original_47"

    full = profile_by_design.get(full_design, {})
    mid = profile_by_design.get(mid_design, {})
    original = profile_by_design.get(original_design, {})

    best_full_faint = full.get("best_faint")
    best_full_bright = full.get("best_bright")
    best_mid_faint = mid.get("best_faint")
    best_original_faint = original.get("best_faint")

    def is_strong(row):
        return bool(
            row
            and row["delta_chi2_score"] >= 25.0
            and row["p_value_chi2_one_dof"] <= 0.01
            and row["observed_exceeds_99_percent_permutation_delta"] is True
        )

    def is_directional(row):
        return bool(
            row
            and row["delta_chi2_score"] >= 10.0
            and row["p_value_chi2_one_dof"] <= 0.05
            and row["observed_exceeds_95_percent_permutation_delta"] is True
        )

    faint_bright_gap = full.get("faint_peak_minus_bright_peak")
    peak_to_median = full.get("faint_peak_to_median_ratio")

    best_cases = {
        "original_design": original,
        "host_measurement_design": mid,
        "full_design": full,
        "best_original_faint": best_original_faint,
        "best_mid_faint": best_mid_faint,
        "best_full_faint": best_full_faint,
        "best_full_bright": best_full_bright,
    }

    if is_strong(best_full_faint) and faint_bright_gap is not None and faint_bright_gap > 20.0 and peak_to_median is not None and peak_to_median >= 1.25:
        return (
            "f160w_faint_threshold_edge_survives_controls",
            8,
            "The F160W faint-end threshold behaves like a residual boundary edge after host, row-order, and measurement controls.",
            best_cases,
        )

    if is_directional(best_full_faint) and faint_bright_gap is not None and faint_bright_gap > 5.0:
        return (
            "f160w_faint_threshold_edge_directional_not_locked",
            7,
            "The F160W faint-end threshold remains directional after controls, but the edge shape is not locked enough for a model.",
            best_cases,
        )

    if best_original_faint and best_full_faint and best_original_faint["delta_chi2_score"] > 25.0 and best_full_faint["delta_chi2_score"] < 10.0:
        return (
            "f160w_threshold_signal_collapses_under_full_controls",
            7,
            "The F160W threshold signal is strong before controls but collapses under host, row-order, and measurement controls.",
            best_cases,
        )

    if best_full_faint and best_full_bright and abs(best_full_faint["delta_chi2_score"] - best_full_bright["delta_chi2_score"]) < 5.0:
        return (
            "f160w_threshold_not_directionally_specific",
            6,
            "Faint-end and bright-end thresholds are too similar; this looks less like a specific faint-end boundary.",
            best_cases,
        )

    return (
        "no_locked_f160w_threshold_edge",
        6,
        "The F160W threshold sweep does not lock a clean faint-end boundary after controls.",
        best_cases,
    )
...
def make_plots(mapped_rows, profile_rows):
    ...
    plt.title("Table2 residuals by mapped row v1.4")
    plt.tight_layout()
    plt.savefig(OUTDIR / "table2_residual_by_row_v1_4.png", dpi=160)
    plt.close()
    ...
    if np.sum(mask) > 50:
        ...
        plt.title("F160W-like magnitude vs absolute residual v1.4")
        plt.tight_layout()
        plt.savefig(OUTDIR / "f160w_vs_abs_residual_v1_4.png", dpi=160)
        plt.close()
    ...
    for design in sorted(set(r["design"] for r in profile_rows)):
        ...
        for side in ["faint_top", "bright_bottom"]:
            ...
            plt.plot(
                [r["percentile"] for r in side_rows],
                [r["delta_chi2_score"] for r in side_rows],
                marker="o",
            )
            plt.xlabel("threshold percentile size")
            plt.ylabel("delta chi2 score")
            plt.title(f"{design}: {side} F160W threshold sweep")
            plt.tight_layout()
            plt.savefig(OUTDIR / f"threshold_sweep_{safe_name(design)}_{side}_v1_4.png", dpi=160)
            plt.close()

    selected_design = "plus_host_top10_row_order_measurement_controls"
    selected_rows = [r for r in profile_rows if r["design"] == selected_design]
    ...
    if selected_rows:
        ...
        for side in ["faint_top", "bright_bottom"]:
            side_rows = sorted(
                [r for r in selected_rows if r["side"] == side],
                key=lambda r: int(r["percentile"]),
            )
        ...
        plt.xlabel("threshold percentile size")
        plt.ylabel("delta chi2 score")
        plt.title("F160W threshold sweep after full controls")
        plt.legend()
        plt.tight_layout()
        plt.savefig(OUTDIR / "threshold_sweep_full_controls_overlay_v1_4.png", dpi=160)
        plt.close()
...
def code_context_search(aux_results):
    ...
    write_csv(OUTDIR / "table2_threshold_context_hits_v1_4.csv", rows)
...
def main():
    print("")
    print("TAIRID Table2 F160W threshold-sweep audit v1.4 starting.")
    print("Boundary: F160W threshold-shape audit only; not proof.")
    print("")
    ...
    write_csv(OUTDIR / "download_ledger_v1_4.csv", ledger)
    write_json(OUTDIR / "download_attempts_v1_4.json", {"compact": downloads, "auxiliary": aux_results})
    ...
    write_json(OUTDIR / "parse_meta_v1_4.json", parse_meta)
    write_json(OUTDIR / "parse_errors_v1_4.json", parse_errors)
    ...
    if parse_errors or not all(key in parsed for key in ["allc", "alll", "ally"]) or not table2_result:
        summary = {
            "test_name": "TAIRID Table2 F160W threshold-sweep audit v1.4",
            "boundary": "Download/parse failure. No threshold-sweep result.",
            "final_status": "table2_f160w_threshold_sweep_v1_4_parse_or_download_failed",
            "readiness_score_0_to_10": 4,
            "next_wall": "Fix compact matrix or table2 retrieval before threshold sweep.",
            "parse_errors": parse_errors,
            "table2_downloaded": bool(table2_result),
        }

        write_json(OUTDIR / "table2_f160w_threshold_sweep_v1_4_summary.json", summary)
    ...
    write_csv(OUTDIR / "compact_row_map_v1_4.csv", row_rows)
    write_csv(OUTDIR / "compact_cluster_map_v1_4.csv", cluster_rows)
    ...
    write_csv(OUTDIR / "design_fit_comparison_v1_4.csv", design_fit_rows)

    candidates = build_threshold_candidates(mapped_rows, len(y))
    ...
    profile_rows, profile_by_design = summarize_threshold_profiles(
        audit_rows,
    ...
    summary = {
        ...
        "boundary": (
            "F160W threshold-shape audit only. Not proof of TAIRID, not H0 resolution, "
        ...
        "threshold_profile_by_design": profile_by_design,
        "threshold_profile_summary": profile_rows,
        "threshold_audit_rows": audit_rows,
        "permutation_control_summaries": permutation_summaries,
        "best_cases": best_cases,
        "code_context_hits_count": len(code_hits),
        "output_files": {
            "summary_json": str(OUTDIR / "table2_f160w_threshold_sweep_v1_4_summary.json"),
            "summary_txt": str(OUTDIR / "table2_f160w_threshold_sweep_v1_4_summary.txt"),
            "mapped_rows_csv": str(OUTDIR / "table2_compact_host_mapped_rows_v1_4.csv"),
            "host_summary_csv": str(OUTDIR / "table2_host_summary_v1_4.csv"),
            "numeric_summary_csv": str(OUTDIR / "table2_numeric_summary_v1_4.csv"),
            "design_fit_comparison_csv": str(OUTDIR / "design_fit_comparison_v1_4.csv"),
            "threshold_audit_csv": str(OUTDIR / "threshold_audit_all_designs_v1_4.csv"),
            "threshold_profile_csv": str(OUTDIR / "threshold_profile_summary_v1_4.csv"),
            "threshold_profile_json": str(OUTDIR / "threshold_profile_by_design_v1_4.json"),
            "permutation_summaries_json": str(OUTDIR / "threshold_permutation_control_summaries_v1_4.json"),
            "permutation_details_csv": str(OUTDIR / "threshold_permutation_control_details_v1_4.csv"),
            "control_design_metadata_json": str(OUTDIR / "control_design_metadata_v1_4.json"),
            "context_hits_csv": str(OUTDIR / "table2_threshold_context_hits_v1_4.csv"),
            "plots": [
                str(OUTDIR / "table2_residual_by_row_v1_4.png"),
                str(OUTDIR / "f160w_vs_abs_residual_v1_4.png"),
                str(OUTDIR / "threshold_sweep_full_controls_overlay_v1_4.png"),
            ],
        },
        "interpretation": {
            "what_supports_boundary_edge": (
                "A faint-end F160W threshold peaks clearly, survives full controls, and beats permutation checks, "
                "while bright-end thresholds remain weaker."
            ),
            "what_supports_ordinary_measurement_behavior": (
                "The threshold signal spreads smoothly across percentiles, appears symmetrically at bright and faint ends, "
                "or collapses under host, row-order, and measurement controls."
            ),
            "truth_boundary": (
                "This test cannot prove TAIRID. It only tests whether the SH0ES Table2 F160W residual signal has "
                "a threshold-boundary shape."
            ),
        },
    }
    ...
    with open(OUTDIR / "table2_f160w_threshold_sweep_v1_4_summary.txt", "w", encoding="utf-8") as f:
        f.write("TAIRID Table2 F160W threshold-sweep audit v1.4\n\n")
        f.write("Boundary: F160W threshold-shape audit only. Not proof. Not H0 resolution.\n\n")
        ...
        f.write("Threshold profile by design:\n")
        f.write(json.dumps(profile_by_design, indent=2, default=json_default) + "\n\n")

        f.write("Threshold profile summary rows:\n")
        ...
        f.write("- This only tests whether the F160W residual signal has a threshold-boundary shape.\n")

    print("")
    print("TAIRID Table2 F160W threshold-sweep audit v1.4 complete.")
    print("Created:")
    print("  tairid_table2_f160w_threshold_sweep_v1_4_outputs/table2_f160w_threshold_sweep_v1_4_summary.json")
    print("  tairid_table2_f160w_threshold_sweep_v1_4_outputs/table2_f160w_threshold_sweep_v1_4_summary.txt")
    print("  tairid_table2_f160w_threshold_sweep_v1_4_outputs/threshold_profile_summary_v1_4.csv")
    print("")
