#!/usr/bin/env python3
"""
TAIRID Cosmology Multi-Surface Pressure Map v3.2.1.1 Fresh
...
v3.2.1 is not a proof test. It is a pressure-map test. This patch fixes the NumPy 2.x trapezoid integration issue from v3.2. It compares several
...
Truth boundary:
v3.2.1 maps pressure surfaces. It does not solve them.
"""
...
OUTDIR = Path("tairid_cosmology_multi_surface_pressure_map_v3_2_1_1_fresh_outputs")
...
# Declared seam constants for pressure-map purposes only.
# These are not fitted here.
H0_EARLY_PLANCK_LIKE = 67.36
H0_EARLY_SIGMA = 0.54
H0_LOCAL_SHOES_LIKE = 73.04
H0_LOCAL_SIGMA = 1.04

# Frozen low-z gate from earlier TAIRID testing. v3.2.1 never fits or promotes it.
FROZEN_GATE_V0_1 = {
...
CLAIMS_V3_2 = {
    "battery_name": "TAIRID Cosmology Multi-Surface Pressure Map v3.2.1.1 Fresh",
...
def fetch_text(repo, branches, repo_path, timeout=120):
    ...
    for branch in branches:
        ...
        try:
            req = urllib.request.Request(url, headers={"User-Agent": "TAIRID-v3.2.1-pressure-map"})
...
def trapezoid_integral(y_values, x_values):
    """NumPy 2.x removed np.trapz in some environments; use np.trapezoid when available, otherwise manual fallback."""
...
def comoving_distance_grid(z_values, om):
    ...
    for z in z_values:
        ...
        dc = C_LIGHT * trapezoid_integral(inv_e, grid) / 70.0
        out.append(dc)
...
def fetch_bao_surfaces():
    ...
    summary = {
        ...
        "truth_boundary": "BAO files are parsed for pressure-surface availability, not fitted as a cosmology likelihood in v3.2.1.",
    }
...
def h0_pressure_seam():
    ...
    return {
        ...
        "truth_boundary": "Declared pressure seam only; v3.2.1 does not solve H0 tension.",
    }
...
def make_plots(sn_diag, pressure_rows):
    try:
        ...
        if len(z) > 0:
            ...
            plt.title("v3.2.1 SN geometry residual pressure surface")
            plt.tight_layout()
            plt.savefig(OUTDIR / "sn_residual_pressure_surface_v3_2_1_1_fresh.png", dpi=160)
            plt.close()
            ...
            plt.title("v3.2.1 diagnostic only: gate pressure vs SN residual")
            plt.tight_layout()
            plt.savefig(OUTDIR / "gate_pressure_vs_sn_residual_v3_2_1_1_fresh.png", dpi=160)
            plt.close()
        ...
        plt.title("v3.2.1 multi-surface pressure map")
        plt.tight_layout()
        plt.savefig(OUTDIR / "multi_surface_pressure_scores_v3_2_1_1_fresh.png", dpi=160)
        plt.close()
    except Exception as exc:
        write_json(OUTDIR / "plot_error_v3_2_1_1_fresh.json", {"error": repr(exc), "traceback": traceback.format_exc()})


def write_handoff(decision):
    lines = []
    lines.append("# TAIRID v3.2.1.1 Fresh Handoff")
    lines.append("")
    ...
    lines.append("")
    lines.append("- v3.2.1 is a pressure map only.")
...
def main():
    print("TAIRID Cosmology Multi-Surface Pressure Map v3.2.1.1 Fresh starting.")
    print("Boundary: pressure map only; no validation, no H0 claim, no new physics claim.")

    write_json(OUTDIR / "claims_v3_2_1_1_fresh.json", CLAIMS_V3_2)
    write_json(OUTDIR / "frozen_gate_v0_1_diagnostic_v3_2_1_1_fresh.json", FROZEN_GATE_V0_1)
    ...
    try:
        ...
        write_csv(OUTDIR / "download_ledger_v3_2_1_1_fresh.csv", download_ledger)
        write_json(OUTDIR / "pantheon_covariance_shape_v3_2_1_1_fresh.json", cov_shape)
        write_csv(OUTDIR / "sn_pressure_diagnostics_v3_2_1_1_fresh.csv", sn_diag)
        write_csv(OUTDIR / "sn_omega_grid_shape_fit_v3_2_1_1_fresh.csv", omega_grid)
        write_json(OUTDIR / "sn_pressure_summary_v3_2_1_1_fresh.json", sn_summary)

        bao_summary, bao_ledger, bao_parsed, bao_previews = fetch_bao_surfaces()
        write_csv(OUTDIR / "bao_download_ledger_v3_2_1_1_fresh.csv", bao_ledger)
        write_csv(OUTDIR / "bao_surface_parse_summary_v3_2_1_1_fresh.csv", bao_parsed)
        write_csv(OUTDIR / "bao_surface_preview_rows_v3_2_1_1_fresh.csv", bao_previews)
        write_json(OUTDIR / "bao_surface_summary_v3_2_1_1_fresh.json", bao_summary)

        h0_summary = h0_pressure_seam()
        preservation_summary = preservation_pressure()
        write_json(OUTDIR / "h0_pressure_seam_v3_2_1_1_fresh.json", h0_summary)
        write_json(OUTDIR / "cmb_bao_preservation_pressure_v3_2_1_1_fresh.json", preservation_summary)

        pressure_rows = score_pressure_surfaces(sn_summary, bao_summary, h0_summary, preservation_summary)
        write_csv(OUTDIR / "multi_surface_pressure_scores_v3_2_1_1_fresh.csv", pressure_rows)

        decision = decide(sn_summary, bao_summary, h0_summary, preservation_summary, pressure_rows)
        write_json(OUTDIR / "decision_v3_2_1_1_fresh.json", decision)

        make_plots(sn_diag, pressure_rows)

        handoff = write_handoff(decision)
        (OUTDIR / "next_thread_handoff_after_v3_2_1_1_fresh.md").write_text(handoff, encoding="utf-8")
        (OUTDIR / "next_thread_handoff_after_v3_2_1_1_fresh.txt").write_text(handoff, encoding="utf-8")

        summary = {
            "test_name": "TAIRID Cosmology Multi-Surface Pressure Map v3.2.1.1 Fresh",
            ...
            "decision": decision,
            "claims_v3_2_1": CLAIMS_V3_2,
            "output_files": {
                "summary_json": str(OUTDIR / "cosmology_multi_surface_pressure_map_v3_2_1_1_fresh_summary.json"),
                "summary_txt": str(OUTDIR / "cosmology_multi_surface_pressure_map_v3_2_1_1_fresh_summary.txt"),
                "download_ledger_csv": str(OUTDIR / "download_ledger_v3_2_1_1_fresh.csv"),
                "sn_pressure_csv": str(OUTDIR / "sn_pressure_diagnostics_v3_2_1_1_fresh.csv"),
                "sn_summary_json": str(OUTDIR / "sn_pressure_summary_v3_2_1_1_fresh.json"),
                "bao_download_ledger_csv": str(OUTDIR / "bao_download_ledger_v3_2_1_1_fresh.csv"),
                "bao_summary_json": str(OUTDIR / "bao_surface_summary_v3_2_1_1_fresh.json"),
                "pressure_scores_csv": str(OUTDIR / "multi_surface_pressure_scores_v3_2_1_1_fresh.csv"),
                "decision_json": str(OUTDIR / "decision_v3_2_1_1_fresh.json"),
                "handoff_md": str(OUTDIR / "next_thread_handoff_after_v3_2_1_1_fresh.md"),
                "plots": [
                    str(OUTDIR / "sn_residual_pressure_surface_v3_2_1_1_fresh.png"),
                    str(OUTDIR / "gate_pressure_vs_sn_residual_v3_2_1_1_fresh.png"),
                    str(OUTDIR / "multi_surface_pressure_scores_v3_2_1_1_fresh.png"),
                ],
            },
            "interpretation": {
                ...
                "next_required_step": "v3.3 may run penalized model comparison only if v3.2.1 passes.",
                "truth_boundary": CLAIMS_V3_2["truth_boundary"],
            },
        }
        write_json(OUTDIR / "cosmology_multi_surface_pressure_map_v3_2_1_1_fresh_summary.json", summary)

        with open(OUTDIR / "cosmology_multi_surface_pressure_map_v3_2_1_1_fresh_summary.txt", "w", encoding="utf-8") as f:
            f.write("TAIRID Cosmology Multi-Surface Pressure Map v3.2.1.1 Fresh\n\n")
        ...
        print("TAIRID Cosmology Multi-Surface Pressure Map v3.2.1.1 Fresh complete.")
        ...
        summary = {
            "test_name": "TAIRID Cosmology Multi-Surface Pressure Map v3.2.1.1 Fresh",
            "created_utc": datetime.now(timezone.utc).isoformat(),
            "final_status": "cosmology_multi_surface_pressure_map_v3_2_1_runtime_failed",
            "error": repr(exc),
