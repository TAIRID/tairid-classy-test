#!/usr/bin/env python3
"""
TAIRID Planck high-l Plik TTTEEE evaluate probe.

Purpose:
Try to evaluate fixed TAIRID proxy candidates against the Planck 2018 high-l
Plik TTTEEE likelihood through Cobaya:

- planck_2018_highl_plik.TTTEEE

Boundary:
This is not the full Planck likelihood.
This does not include low-l TT/EE or lensing in the same score yet.
This does not run MCMC.
This does not explore nuisance parameters.
This does not prove TAIRID cosmology.
This is a high-l likelihood bridge test.
"""
...
OUTDIR = Path("planck_high_l_plik_evaluate_outputs")
OUTDIR.mkdir(parents=True, exist_ok=True)

RUN_DIR = OUTDIR / "candidate_yamls"
RUN_DIR.mkdir(parents=True, exist_ok=True)

PACKAGES_DIR = Path("cobaya_planck_high_l_packages_runtime")
...
REPORT = {
    "boundary": "Planck high-l Plik TTTEEE evaluate probe only. Not full Planck and not proof of TAIRID cosmology.",
    "python": {
        "version": sys.version,
...
def install_planck_high_l_component():
    component = "planck_2018_highl_plik.TTTEEE"
...
def evaluate_candidate(case):
    ...
    for line in combined_text.splitlines():
        low = line.lower()
        if "logpost" in low or "loglike" in low or "chi2" in low or "minuslogpost" in low:
            likelihood_lines.append(line)

    result["likelihood_related_lines"] = likelihood_lines[-80:]

    return result


def extract_simple_chi2(case_result):
    lines = case_result.get("likelihood_related_lines", [])
    values = []

    for line in lines:
        lowered = line.lower()
        if "chi2" not in lowered:
            continue
...
def main():
    ...
    install_planck_high_l_component()

    for case in CASES:
        print("")
        print("Evaluating high-l candidate:", case["name"])

        try:
            result = evaluate_candidate(case)
            result["simple_extracted_chi2_candidate"] = extract_simple_chi2(result)
        except Exception as exc:
            result = {
                "name": case["name"],
                "label": case["label"],
    ...
    if passed_cases:
        REPORT["interpretation"] = (
            "At least one fixed candidate evaluated successfully against Planck high-l Plik TTTEEE. "
            "Inspect likelihood-related lines and compare chi2 carefully."
        )
    else:
        REPORT["interpretation"] = (
            "No candidate completed high-l Plik evaluation successfully. This is a bridge failure, not a cosmology failure. "
            "Inspect stderr tails for nuisance, foreground, or likelihood-path issues."
        )

    report_path = OUTDIR / "planck_high_l_plik_evaluate_report.json"
    report_path.write_text(json.dumps(REPORT, indent=2))

    with open(OUTDIR / "planck_high_l_plik_evaluate_summary.txt", "w") as f:
        f.write("TAIRID Planck high-l Plik TTTEEE evaluate probe\n")
        f.write("\n")
        f.write("Boundary: not full Planck, not proof of TAIRID cosmology.\n")
        f.write("\n")
        ...
        for case in REPORT["cases"]:
            f.write(
                f"{case['name']}: {case['status']} returncode={case.get('returncode')} "
                f"simple_chi2={case.get('simple_extracted_chi2_candidate')}\n"
            )
    ...
    print("")
    print("TAIRID Planck high-l Plik evaluate probe complete.")
    print("Created:")
    print("  planck_high_l_plik_evaluate_outputs/planck_high_l_plik_evaluate_report.json")
    print("  planck_high_l_plik_evaluate_outputs/planck_high_l_plik_evaluate_summary.txt")
    print("")
    print("Boundary:")
    print("  This is not full Planck.")
    print("  This is the high-l Plik TTTEEE likelihood bridge test.")
