#!/usr/bin/env python3
"""
TAIRID growth history scan v2.

Purpose:
The refined warm-neutral corridor scan found a best candidate near:

    warm fraction = 7.5 percent
    proxy mass    = 25 eV

This test checks whether the same candidates have a reasonable growth history.
...
CASES = [
    ...
    {
        "name": "candidate_10pct_35eV",
        "warm_fraction": 0.10,
        "m_ncdm_eV": 35.0,
    },
    {
        "name": "candidate_12p5pct_40eV",
        "warm_fraction": 0.125,
        "m_ncdm_eV": 40.0,
    },
    {
...
def sigma_R_from_pk(k_values, pk_values, R_mpc):
    ...
    sigma2 = np.trapz(integrand, ln_k)
...
def run_class_case(params):
    ...
    return class_sigma8_z0, ell, dl, sigma8_values, pk_by_z
...
for case in CASES:
    ...
    try:
        class_sigma8_z0, ell, dl, sigma8_values, pk_by_z = run_class_case(params)
...
Path("growth_history_scan_v2_summary.json").write_text(json.dumps(results, indent=2))


with open("growth_history_scan_v2_summary.csv", "w", newline="") as f:
    writer = csv.writer(f)
...
if growth_tables:
    ...
    plt.title("Growth history scan v2: sigma8(z)")
    plt.legend(fontsize=7)
    plt.tight_layout()
    plt.savefig("growth_history_v2_sigma8_z.png", dpi=160)
    plt.close()
    ...
    plt.title("Growth history scan v2: f sigma8(z)")
    plt.legend(fontsize=7)
    plt.tight_layout()
    plt.savefig("growth_history_v2_fsigma8_z.png", dpi=160)
    plt.close()
...
if "cdm_baseline" in growth_tables:
    ...
    plt.title("Growth history scan v2: f sigma8 ratio")
    plt.legend(fontsize=7)
    plt.tight_layout()
    plt.savefig("growth_history_v2_fsigma8_ratio.png", dpi=160)
    plt.close()
    ...
    plt.title("Growth history scan v2: sigma8 ratio")
    plt.legend(fontsize=7)
    plt.tight_layout()
    plt.savefig("growth_history_v2_sigma8_ratio.png", dpi=160)
    plt.close()


print("")
print("Growth history scan v2 complete.")
print("Created:")
print("  growth_history_scan_v2_summary.json")
print("  growth_history_scan_v2_summary.csv")
print("  growth_history_v2_sigma8_z.png")
print("  growth_history_v2_fsigma8_z.png")
print("  growth_history_v2_fsigma8_ratio.png")
print("  growth_history_v2_sigma8_ratio.png")
