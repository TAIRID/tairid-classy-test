#!/usr/bin/env python3
"""
TAIRID ETDD70 response-shape test v3.

Purpose:
The SH0ES compact-ladder tests showed that simple offset/gate translations are too easily
absorbed by ordinary model freedom. The next TAIRID translation test should look for a
dynamic response shape: pacing, constraint, mismatch, and recovery/curvature under task
demand.

This v3 script fixes the v1/v2 parser failures:
- v1 failed because mixed inventory rows had different CSV fields.
- v2 failed because task_rank was accidentally aggregated twice.
- v3 uses robust CSV writing and excludes grouping columns from aggregation.

Dataset:
ETDD70 Eye-Tracking Dyslexia Dataset, Zenodo record 13332134.

Question:
Does a TAIRID pacing-constraint mismatch response-shape model carry group-relevant signal
beyond simple static mean differences?

Boundary:
This is not proof of TAIRID.
This is not clinical diagnosis.
This is not a cosmology result.
It is a cross-domain response-shape test.
"""
...
OUTDIR = Path("tairid_etdd70_response_shape_v3_outputs")
...
COLUMN_HINTS = {
    "subject": ["subject", "participant", "subj", "id", "child", "student", "pupil"],
    "group": ["group", "class", "label", "diagnosis", "dyslexia", "reader", "type"],
    "task": ["task", "text", "stimulus", "trial", "condition", "reading"],
    "time": ["time", "timestamp", "t", "ms", "sec", "second"],
    "x": ["x", "gaze_x", "gazex", "fix_x", "fixx", "posx", "screen_x"],
    "y": ["y", "gaze_y", "gazey", "fix_y", "fixy", "posy", "screen_y"],
    "duration": ["duration", "dur", "fixation_duration", "fixdur", "fix_dur", "dwell"],
    "saccade": ["saccade", "sacc", "amplitude", "velocity", "sacamp"],
    "event": ["event", "type", "event_type", "label"],
}
...
def write_csv(path, rows):
    ...
    Rows from downloads, extraction, inventory, and feature parsing can have different
    fields. This builds a stable union of all keys instead of assuming every row has
    the same structure as the first row.
    """
    if not rows:
...
def read_url(url, timeout=240):
    req = urllib.request.Request(
        url,
        headers={
            "User-Agent": "TAIRID-ETDD70-response-shape-v3",
...
def infer_subject_from_path(path):
    joined = "/".join([p.lower() for p in Path(path).parts])
...
def load_group_label_map():
    """
    ETDD70 includes dyslexia_class_label.csv. This function tries to build a subject -> group map
    from any downloaded/extracted file with label/group/dyslexia columns.
    """
    label_map = {}
    ...
    for root in [DOWNLOAD_DIR, EXTRACT_DIR]:
        for path in root.rglob("*"):
            ...
            if "label" not in lower_name and "class" not in lower_name and "dys" not in lower_name:
                continue

            if not lower_name.endswith((".csv", ".tsv", ".txt", ".xlsx", ".xls")):
                continue
            ...
            sub_col = find_column(df.columns, "subject")
            group_col = find_column(df.columns, "group")
            ...
            for _, row in df.iterrows():
                subj = str(row.get(sub_col, "")).strip()
...
def table_inventory_and_features(label_map):
    ...
    for path in table_paths:
        ...
        if group is None and subject is not None:
            group = label_map.get(str(subject))
            if group is None:
                m = re.search(r"([0-9]{1,4})", str(subject))
                if m:
                    group = label_map.get(m.group(1))
...
def aggregate_subject_task_features(feature_rows):
    """
    Aggregate file-level/event-level features into subject x task features.

    v3 fix:
    Exclude grouping columns from numeric_cols. In v2, task_rank was numeric and
    accidentally included in both the groupby keys and the aggregated numeric columns,
    causing pandas to fail at reset_index().
    """
    if not feature_rows:
        return pd.DataFrame()
    ...
    numeric_cols = [
        ...
        if c not in ["source_file", "subject", "group", "task_rank"]
...
def build_tairid_shape_features(agg):
        ...
        subject_rows.append(
            {
                ...
                "T_slope_over_task": T_slope,
                "I_slope_over_task": I_slope,
    ...
    return df, subject_df, usable_cols
...
def simple_logistic_cv(df, feature_cols, label_col="label", k=5):
    ...
    if len(data) < 20 or data[label_col].nunique() < 2:
        return {"status": "not_enough_data", "n": int(len(data))}
...
def analyze_response_shape(subject_df):
    results = {
        "status": "started",
        "feature_tests": [],
        "models": {},
...
def plot_outputs(task_df, subject_df):
    ...
    if not task_df.empty and "T_pacing_proxy" in task_df.columns:
        ...
        plt.title("TAIRID ETDD70 response shape v3: mismatch across task demand")
        plt.legend()
        plt.tight_layout()
        path = OUTDIR / "etdd70_mismatch_across_task_v3.png"
        ...
        plt.title("TAIRID ETDD70 response shape v3: collapse load across task demand")
        plt.legend()
        plt.tight_layout()
        path = OUTDIR / "etdd70_collapse_load_across_task_v3.png"
        plt.savefig(path, dpi=160)
        plt.close()
        plots.append(str(path))
    ...
    if not subject_df.empty and "mismatch_slope_over_task" in subject_df.columns:
        ...
        if values:
            ...
            plt.title("TAIRID ETDD70 response shape v3: subject-level mismatch slope")
            plt.tight_layout()
            path = OUTDIR / "etdd70_subject_mismatch_slope_boxplot_v3.png"
            plt.savefig(path, dpi=160)
            plt.close()
            plots.append(str(path))
...
def main():
    print("")
    print("TAIRID ETDD70 response-shape test v3 starting.")
    ...
    write_csv(OUTDIR / "etdd70_download_ledger_v3.csv", downloads)
    write_csv(OUTDIR / "etdd70_extraction_ledger_v3.csv", extraction)

    label_map = load_group_label_map()
    label_map_rows = [{"subject_key": k, "group": v} for k, v in sorted(label_map.items())]
    write_csv(OUTDIR / "etdd70_label_map_v3.csv", label_map_rows)

    inventory, feature_rows = table_inventory_and_features(label_map)

    inventory_path = write_csv(OUTDIR / "etdd70_table_inventory_v3.csv", inventory)
    features_path = write_csv(OUTDIR / "etdd70_raw_feature_rows_v3.csv", feature_rows)

    subject_task = aggregate_subject_task_features(feature_rows)
    if not subject_task.empty:
        subject_task_path = OUTDIR / "etdd70_subject_task_features_v3.csv"
        subject_task.to_csv(subject_task_path, index=False)
    else:
        subject_task_path = None

    task_shape_df, subject_shape_df, usable_cols = build_tairid_shape_features(subject_task)

    if not task_shape_df.empty:
        task_shape_path = OUTDIR / "etdd70_tairid_task_shape_features_v3.csv"
        task_shape_df.to_csv(task_shape_path, index=False)
    else:
        task_shape_path = None

    if not subject_shape_df.empty:
        subject_shape_path = OUTDIR / "etdd70_tairid_subject_response_shape_v3.csv"
        subject_shape_df.to_csv(subject_shape_path, index=False)
    else:
        subject_shape_path = None

    analysis = analyze_response_shape(subject_shape_df)

    analysis_path = OUTDIR / "etdd70_response_shape_analysis_v3.json"
    analysis_path.write_text(json.dumps(analysis, indent=2), encoding="utf-8")

    plots = plot_outputs(task_shape_df, subject_shape_df)

    if analysis.get("status") != "ok":
        final_status = "data_inventory_complete_response_shape_not_estimated"
        readiness_score = 5
        next_wall = "Inspect table inventory and update parser for ETDD70 file structure."
    else:
        ...
        if dyn_auc is not None and static_auc is not None and dyn_auc > static_auc + 0.05:
            ...
            readiness_score = 7
            next_wall = "Check robustness and compare to simpler non-TAIRID features."
        else:
    ...
    summary = {
        "test_name": "TAIRID ETDD70 response-shape test v3",
        "boundary": (
        ...
        "usable_feature_columns": usable_cols,
        "analysis": analysis,
        "output_files": {
            "download_ledger_csv": str(OUTDIR / "etdd70_download_ledger_v3.csv"),
            "extraction_ledger_csv": str(OUTDIR / "etdd70_extraction_ledger_v3.csv"),
            "label_map_csv": str(OUTDIR / "etdd70_label_map_v3.csv"),
        ...
        "interpretation": {
            "what_would_support_TAIRID_shape": (
                "Dynamic mismatch, curvature, or slope features carry signal beyond static mean-load proxies."
            ),
            "what_would_weaken_this_translation": (
                "Only simple mean differences appear, or event-level task/timing structure cannot be recovered."
            ),
            "translation_rule": (
                "Translate operator roles, not objects: pacing, constraint, mismatch, stability, boundary transition."
            ),
        },
    }

    summary_path = OUTDIR / "etdd70_response_shape_v3_summary.json"
    summary_path.write_text(json.dumps(summary, indent=2), encoding="utf-8")

    with open(OUTDIR / "etdd70_response_shape_v3_summary.txt", "w", encoding="utf-8") as f:
        f.write("TAIRID ETDD70 response-shape test v3\n\n")
    ...
    print("")
    print("TAIRID ETDD70 response-shape test v3 complete.")
    print("Created:")
    print("  tairid_etdd70_response_shape_v3_outputs/etdd70_response_shape_v3_summary.json")
    print("  tairid_etdd70_response_shape_v3_outputs/etdd70_response_shape_v3_summary.txt")
    print("  tairid_etdd70_response_shape_v3_outputs/etdd70_table_inventory_v3.csv")
    print("  tairid_etdd70_response_shape_v3_outputs/etdd70_raw_feature_rows_v3.csv")
    print("  tairid_etdd70_response_shape_v3_outputs/etdd70_response_shape_analysis_v3.json")
    print("")
    print("Boundary:")
