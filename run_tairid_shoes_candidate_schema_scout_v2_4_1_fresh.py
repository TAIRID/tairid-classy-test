#!/usr/bin/env python3
"""
TAIRID SH0ES Candidate Schema Scout v2.4.1 Fresh

Fresh-file repair build.

Why this file exists:
The prior run got crossed with an unrelated CLASS/classy workflow. This file
uses brand-new names everywhere: workflow name, script filename, output folder,
summary filenames, and artifact name. Do not edit or reuse old v2.4 files.

Purpose:
v2.4.1 is still only a schema scout. It inspects selected public
Pantheon+SH0ES/DataRelease files to decide what the next legitimate test can be.

It DOES:
    - download selected downstream distance/covariance files,
    - inspect redshift/host metadata files,
    - inspect SH0ES optical Cepheid measurement files,
    - inspect one simulation FITRES file only as a schema-control lane,
    - classify candidate surfaces as downstream outcome, covariance, metadata,
      adjacent measurement, simulation-only, or unusable,
    - decide whether v2.5 should build a bridge before replay.
...
It DOES NOT:
    ...
    - replay the locked F160W residual edge.

Truth boundary:
This is schema scouting only. Success means the next lane is better defined.
It is not model validation.
"""
...
OUTDIR = Path("tairid_shoes_candidate_schema_scout_v2_4_1_fresh_outputs")
...
CLAIMS_V2_4_1 = {
    "battery_name": "TAIRID SH0ES Candidate Schema Scout v2.4.1 Fresh",
    "scope": "Fresh-file schema inspection for external/downstream candidate lanes after v2.3 provenance inventory",
    "primary_question": (
        "Which candidate files contain real downstream outcome, covariance, metadata, "
        "or simulation-only schemas, and what next test is allowed without tuning the frozen F160W rule?"
    ),
    "truth_boundary": (
...
FROZEN_RULE_CARRY_FORWARD = {
    ...
    "hard_boundary": [
        "Do not tune the frozen rule in v2.4.1.",
        "Do not run a replay in v2.4.1.",
...
def fetch_url_bytes(url):
    req = urllib.request.Request(
        url,
        headers={"User-Agent": "TAIRID-v2.4.1-fresh-schema-scout"},
    )
...
def detect_semantic_columns(columns):
    ...
    return {
        ...
        "has_distance_like": any("dist" in c or c == "dl" for c in low),
...
def parse_snana_or_table_text(text, max_rows=10000):
    ...
    for raw in lines:
        ...
        if first_noncomment is None:
            ...
            numeric_ratio = (
                sum(token_is_number(t) for t in maybe_cols) / len(maybe_cols)
                if maybe_cols
                else 0
            )
...
def parse_cov_text(text):
    ...
    return {
        ...
        "numeric_first_50_count": sum(1 for t in tokens[:50] if token_is_number(t)),
        "semantic_flags": {
...
def choose_next_lane(inspections):
    ...
    downstream_ok = bool(distance and distance.get("downstream_outcome_possible"))
    cov_ok = bool(statsys and statsys.get("covariance_only") and statsys.get("cov_has_expected_square_payload") is True)
    statonly_ok = bool(statonly and statonly.get("covariance_only") and statonly.get("cov_has_expected_square_payload") is True)
    no_direct_table2_keys = bool(distance and not distance.get("has_table2_direct_keys"))
    sim_not_validation = bool(sim and sim.get("simulation_only"))
    ...
    gates = [
        {
            ...
            "evidence": {
                "distance_columns": distance.get("columns_detected") if distance else None,
                "has_mu_like": distance.get("has_mu_like") if distance else None,
                "has_redshift_like": distance.get("has_redshift_like") if distance else None,
                "parsed_preview_row_count": distance.get("parsed_preview_row_count") if distance else None,
            },
        },
        {
        ...
        },
        {
            ...
            "evidence": {
                "has_table2_direct_keys": distance.get("has_table2_direct_keys") if distance else None,
        ...
        },
        {
            ...
            "evidence": {
                "simulation_only": sim.get("simulation_only") if sim else None,
                "recommended_use": sim.get("recommended_use") if sim else None,
            },
        },
    ]
    ...
    return {
        ...
        "gates": gates,
        "failed_gates": failed,
        "truth_boundary": CLAIMS_V2_4_1["truth_boundary"],
    }
...
def holographic_surface_ledger(decision, inspections):
    return {
        "observable_surface": {
            "name": "Fresh v2.4.1 selected candidate schemas after v2.3 provenance inventory",
            "repo": REPO,
            "targets": TARGETS,
        },
        "hidden_depth_sought": {
            "name": "Whether an external/downstream residual or outcome lane exists",
            "allowed_claim": "v2.4.1 can identify downstream outcome/covariance schemas and whether a bridge is required.",
            "not_allowed_claim": "v2.4.1 cannot validate TAIRID, replay the frozen rule, or claim H0 correction.",
        },
        "boundary_that_forms_surface": {
            "locked_lane_boundary": "v1.9-v2.2 Table2 residual edge lane remains frozen",
            "schema_boundary": "v2.4.1 inspects file columns and covariance shapes only",
...
def write_handoff(decision, inspections):
    ...
    lines = []
    lines.append("# TAIRID v2.4.1 Fresh Handoff")
    lines.append("")
    ...
    lines.append("")
    lines.append("## What v2.4.1 did")
    lines.append("")
    lines.append("- Used brand-new workflow/script/output names to avoid collisions with prior files.")
    ...
    lines.append("")
    lines.append("- v2.4.1 is schema scouting only.")
    ...
    if decision["readiness_score_0_to_10"] >= 8:
        lines.append(
            "v2.5 should build a calibrator/host bridge and downstream influence audit. "
            "It should not directly replay the F160W edge against Pantheon+SH0ES distances until the bridge is proven."
        )
    else:
        lines.append(
...
def main():
    print("TAIRID SH0ES Candidate Schema Scout v2.4.1 Fresh starting.")
    print("Boundary: schema scouting only; no replay, no tuning, no H0 claim.")
    print("Fresh output folder:", OUTDIR)

    write_json(OUTDIR / "claims_v2_4_1_fresh.json", CLAIMS_V2_4_1)
    write_json(OUTDIR / "frozen_rule_carry_forward_v2_4_1_fresh.json", FROZEN_RULE_CARRY_FORWARD)
    ...
    try:
        ...
        for target in TARGETS:
            ...
            write_json(OUTDIR / f"{target['label']}_schema_detail_v2_4_1_fresh.json", inspection)

            preview_rows = inspection.get("parsed_detail", {}).get("preview_rows", [])
            if preview_rows and isinstance(preview_rows, list):
                write_csv(OUTDIR / f"{target['label']}_preview_rows_v2_4_1_fresh.csv", preview_rows)

        write_csv(OUTDIR / "download_ledger_v2_4_1_fresh.csv", download_ledger)
        ...
        write_csv(OUTDIR / "candidate_schema_inventory_v2_4_1_fresh.csv", flat_inspections)

        decision = choose_next_lane(inspections)
        write_json(OUTDIR / "decision_v2_4_1_fresh.json", decision)

        ledger = holographic_surface_ledger(decision, inspections)
        write_json(OUTDIR / "holographic_surface_ledger_v2_4_1_fresh.json", ledger)

        handoff = write_handoff(decision, inspections)
        (OUTDIR / "next_thread_handoff_after_v2_4_1_fresh.md").write_text(handoff, encoding="utf-8")
        (OUTDIR / "next_thread_handoff_after_v2_4_1_fresh.txt").write_text(handoff, encoding="utf-8")
        ...
        summary = {
            ...
            "decision": decision,
            "claims_v2_4_1": CLAIMS_V2_4_1,
            "frozen_rule_carry_forward": FROZEN_RULE_CARRY_FORWARD,
            "holographic_surface_ledger": ledger,
            "output_files": {
                "summary_json": str(OUTDIR / "shoes_candidate_schema_scout_v2_4_1_fresh_summary.json"),
                "summary_txt": str(OUTDIR / "shoes_candidate_schema_scout_v2_4_1_fresh_summary.txt"),
                "download_ledger_csv": str(OUTDIR / "download_ledger_v2_4_1_fresh.csv"),
                "schema_inventory_csv": str(OUTDIR / "candidate_schema_inventory_v2_4_1_fresh.csv"),
                "decision_json": str(OUTDIR / "decision_v2_4_1_fresh.json"),
                "ledger_json": str(OUTDIR / "holographic_surface_ledger_v2_4_1_fresh.json"),
                "handoff_md": str(OUTDIR / "next_thread_handoff_after_v2_4_1_fresh.md"),
                "handoff_txt": str(OUTDIR / "next_thread_handoff_after_v2_4_1_fresh.txt"),
            },
            "interpretation": {
        ...
        write_json(OUTDIR / "shoes_candidate_schema_scout_v2_4_1_fresh_summary.json", summary)

        with open(OUTDIR / "shoes_candidate_schema_scout_v2_4_1_fresh_summary.txt", "w", encoding="utf-8") as f:
            f.write("TAIRID SH0ES Candidate Schema Scout v2.4.1 Fresh\n\n")
        ...
        print("TAIRID SH0ES Candidate Schema Scout v2.4.1 Fresh complete.")
        ...
        summary = {
            "test_name": "TAIRID SH0ES Candidate Schema Scout v2.4.1 Fresh",
            "created_utc": datetime.now(timezone.utc).isoformat(),
            "final_status": "shoes_candidate_schema_scout_v2_4_1_fresh_runtime_failed",
            "error": repr(exc),
