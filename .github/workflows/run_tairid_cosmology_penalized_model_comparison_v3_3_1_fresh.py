#!/usr/bin/env python3
"""
TAIRID Cosmology Penalized Model Comparison v3.3.1 Fresh

Purpose:
Run a compact penalized SN-surface prototype after the v3.0-v3.2 reset.
Compare three metric-preserving distance-shape models on Pantheon+SH0ES:
...
show standard cosmology is wrong. It only asks whether the frozen gate earns
follow-up after AIC/BIC penalties. If it does not, stop using it as an SN
...
OUTDIR = Path("tairid_cosmology_penalized_model_comparison_v3_3_1_fresh_outputs")
...
CLAIMS = {
    "test_name": "TAIRID Cosmology Penalized Model Comparison v3.3.1 Fresh",
...
def fetch_text(repo: str, branches: list[str], repo_path: str, timeout: int = 120) -> dict:
    ...
    for branch in branches:
        ...
        try:
            req = urllib.request.Request(url, headers={"User-Agent": "TAIRID-v3.3.1-penalized-model-comparison"})
...
def select_sn_rows(rows: list[dict]) -> list[dict]:
    selected = []
    for row in rows:
        z, mu, sig = row.get("zHD"), row.get("MU_SH0ES"), row.get("MU_SH0ES_ERR_DIAG")
...
def fetch_bao_availability():
    ...
    for target in BAO_TARGETS:
        ...
        rows.append({
            ...
            "truth_boundary": "Availability check only; BAO is not fitted in v3.3.1.",
        })
...
def decide(comparison, bao_rows, cov_shape):
    ...
    gates = [
        {"gate": "G1_sn_surface_fit_completed", "passed": all(m.get("n", 0) >= 1000 for m in comparison), "evidence": {m["model"]: m.get("n") for m in comparison}},
        {"gate": "G2_pantheon_covariance_shape_available", "passed": cov_ok, "evidence": cov_shape},
        {"gate": "G3_bao_surface_lock_available", "passed": bao_available, "evidence": {"downloaded": sum(1 for r in bao_rows if r["status"] == "downloaded"), "target_count": len(bao_rows)}},
        {"gate": "G4_tairid_fixed_gate_survives_lcdm_penalty", "passed": tairid_clears_lcdm, "evidence": {"delta_bic_tairid_minus_lcdm": delta_bic_tairid_minus_lcdm, "delta_aic_tairid_minus_lcdm": delta_aic_tairid_minus_lcdm, "threshold": "BIC must be <= -6"}},
        {"gate": "G5_tairid_fixed_gate_competitive_with_wcdm", "passed": tairid_competes_wcdm, "evidence": {"delta_bic_tairid_minus_wcdm": delta_bic_tairid_minus_wcdm, "threshold": "BIC must be <= 2"}},
...
def make_plots(comparison, residuals):
    try:
        ...
        plt.title("v3.3.1 penalized model comparison")
        plt.tight_layout()
        plt.savefig(OUTDIR / "model_delta_bic_v3_3_1_fresh.png", dpi=160)
        plt.close()
        ...
        plt.title("v3.3.1 SN residual comparison")
        plt.legend()
        plt.tight_layout()
        plt.savefig(OUTDIR / "sn_residual_model_comparison_v3_3_1_fresh.png", dpi=160)
        plt.close()
        ...
        plt.title("v3.3.1 gate basis vs LCDM residual")
        plt.tight_layout()
        plt.savefig(OUTDIR / "gate_basis_vs_lcdm_residual_v3_3_1_fresh.png", dpi=160)
        plt.close()
    except Exception as exc:
        write_json(OUTDIR / "plot_error_v3_3_1_fresh.json", {"error": repr(exc), "traceback": traceback.format_exc()})


def write_handoff(decision):
    lines = [
        "# TAIRID v3.3.1 Fresh Handoff",
        "",
        "## Current status",
        "",
    ...
    for model in decision["model_ranking_by_bic"]:
        lines.append(f"- `{model['model']}`: BIC `{model['bic']:.6f}`, ΔBIC `{model['delta_bic_vs_best']:.6f}`, chi2 `{model['chi2']:.6f}`, params `{model['k_params']}`")
    lines.extend([
        "",
        "## Truth boundary",
        "",
        "- v3.3.1 is a penalized SN-surface prototype only.",
...
def main():
    print("TAIRID Cosmology Penalized Model Comparison v3.3.1 Fresh starting.")
    print("Boundary: penalized SN-surface prototype only; no validation, no H0 claim, no new physics claim.")

    write_json(OUTDIR / "claims_v3_3_1_fresh.json", CLAIMS)
    write_json(OUTDIR / "frozen_gate_v0_1_penalty_test_v3_3_1_fresh.json", FROZEN_GATE)
    ...
    try:
        ...
        write_csv(OUTDIR / "download_ledger_v3_3_1_fresh.csv", [
        ...
        write_json(OUTDIR / "pantheon_covariance_shape_v3_3_1_fresh.json", cov_shape)
        ...
        write_csv(OUTDIR / "model_comparison_v3_3_1_fresh.csv", comparison)
        write_csv(OUTDIR / "lcdm_grid_scores_v3_3_1_fresh.csv", lcdm_grid)
        write_csv(OUTDIR / "wcdm_grid_scores_v3_3_1_fresh.csv", wcdm_grid)
        write_csv(OUTDIR / "tairid_fixed_gate_grid_scores_v3_3_1_fresh.csv", tairid_grid)

        residuals = residual_table(sn_rows, z, mu, weights, {"lcdm": lcdm_obj, "wcdm": wcdm_obj, "tairid": tairid_obj})
        write_csv(OUTDIR / "best_model_residuals_v3_3_1_fresh.csv", residuals)

        bao_rows = fetch_bao_availability()
        write_csv(OUTDIR / "bao_availability_v3_3_1_fresh.csv", bao_rows)

        decision = decide(comparison, bao_rows, cov_shape)
        write_json(OUTDIR / "decision_v3_3_1_fresh.json", decision)
        make_plots(comparison, residuals)

        handoff = write_handoff(decision)
        (OUTDIR / "next_thread_handoff_after_v3_3_1_fresh.md").write_text(handoff, encoding="utf-8")
        (OUTDIR / "next_thread_handoff_after_v3_3_1_fresh.txt").write_text(handoff, encoding="utf-8")
        ...
        summary = {
            ...
            "bao_availability": {"target_count": len(bao_rows), "downloaded_count": sum(1 for r in bao_rows if r["status"] == "downloaded")},
            "decision": decision,
            "output_files": {
                "summary_json": str(OUTDIR / "cosmology_penalized_model_comparison_v3_3_1_fresh_summary.json"),
                "summary_txt": str(OUTDIR / "cosmology_penalized_model_comparison_v3_3_1_fresh_summary.txt"),
                "model_comparison_csv": str(OUTDIR / "model_comparison_v3_3_1_fresh.csv"),
                "residuals_csv": str(OUTDIR / "best_model_residuals_v3_3_1_fresh.csv"),
                "decision_json": str(OUTDIR / "decision_v3_3_1_fresh.json"),
                "handoff_md": str(OUTDIR / "next_thread_handoff_after_v3_3_1_fresh.md"),
            },
            "interpretation": {
        ...
        write_json(OUTDIR / "cosmology_penalized_model_comparison_v3_3_1_fresh_summary.json", summary)
        with open(OUTDIR / "cosmology_penalized_model_comparison_v3_3_1_fresh_summary.txt", "w", encoding="utf-8") as f:
            f.write("TAIRID Cosmology Penalized Model Comparison v3.3.1 Fresh\n\n")
        ...
        print("TAIRID Cosmology Penalized Model Comparison v3.3.1 Fresh complete.")
        ...
        summary = {
            "test_name": CLAIMS["test_name"],
            "created_utc": datetime.now(timezone.utc).isoformat(),
            "final_status": "cosmology_penalized_model_comparison_v3_3_1_runtime_failed",
            "error": repr(exc),
