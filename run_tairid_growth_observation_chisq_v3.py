#!/usr/bin/env python3
"""
TAIRID observed growth / f_sigma8(z) chi-square pressure test v3.

Purpose:
Compare TAIRID warm-neutral proxy candidates against a simple observed
f_sigma8(z) redshift-space-distortion dataset.

Boundary:
This is a diagnostic proxy test, not a final survey likelihood.
The observed table is diagonal-only and ignores covariance between some points.
The non-cold neutral component is represented by CLASS ncdm.
"""
...
def interp_model_at_obs(z_grid, y_grid):
...
for case in CASES:
    ...
    try:
        ...
        model_fs8_obs = interp_model_at_obs(Z_GRID, f_sigma8)
        ...
        obs_file = f"{name}_obs_residuals.txt"
        np.savetxt(
            obs_file,
            np.column_stack(
                [
                    OBS_Z,
                    np.array([x["fs8"] for x in OBS_FSIGMA8]),
                    np.array([x["sigma"] for x in OBS_FSIGMA8]),
                    model_fs8_obs,
                    model_fs8_obs - np.array([x["fs8"] for x in OBS_FSIGMA8]),
                ]
            ),
            header="z observed_fs8 sigma model_fs8 residual_model_minus_obs",
        )
...
Path("growth_observation_chisq_v3_summary.json").write_text(json.dumps(results, indent=2))


with open("growth_observation_chisq_v3_summary.csv", "w", newline="") as f:
    writer = csv.writer(f)
...
with open("growth_observation_residuals_v3.csv", "w", newline="") as f:
    writer = csv.writer(f)
...
with open("growth_observation_model_curves_v3.csv", "w", newline="") as f:
    writer = csv.writer(f)
...
if success_names:
    ...
    plt.ylabel("chi2 against observed f_sigma8 proxy")
    plt.title("Growth observation chi-square v3 by case")
    plt.tight_layout()
    plt.savefig("growth_observation_chi2_by_case_v3.png", dpi=160)
    plt.close()
    ...
    plt.title("Model growth curves against observed proxy table")
    plt.legend(fontsize=7)
    plt.tight_layout()
    plt.savefig("growth_observation_fsigma8_curves_v3.png", dpi=160)
    plt.close()

    sorted_success = sorted(
        success_names,
        key=lambda n: results[n]["chi2_growth_observed"],
    )
    best_name = sorted_success[0]

    plt.figure(figsize=(11, 5))
    for name in ["cdm_baseline", best_name]:
        if name not in growth_tables:
            continue

        model = interp_model_at_obs(Z_GRID, growth_tables[name]["f_sigma8"])
        pulls = (model - obs_y) / obs_err
        plt.plot(obs_z, pulls, marker="o", label=name)

    plt.axhline(0.0, linewidth=1)
    plt.axhline(1.0, linewidth=1)
    plt.axhline(-1.0, linewidth=1)
    plt.xlabel("z")
    plt.ylabel("pull in sigma")
    plt.title("Growth observation residual pulls v3")
    plt.legend(fontsize=8)
    plt.tight_layout()
    plt.savefig("growth_observation_residual_pulls_v3.png", dpi=160)
    plt.close()


print("")
print("Growth observation chi-square v3 complete.")
print("Created:")
print("  growth_observation_chisq_v3_summary.json")
print("  growth_observation_chisq_v3_summary.csv")
print("  growth_observation_residuals_v3.csv")
print("  growth_observation_model_curves_v3.csv")
print("  growth_observation_chi2_by_case_v3.png")
print("  growth_observation_fsigma8_curves_v3.png")
print("  growth_observation_residual_pulls_v3.png")
