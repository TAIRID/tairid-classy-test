#!/usr/bin/env python3
"""
TAIRID Table2 host residual audit v1.2.

Purpose:
This is the next clean SH0ES Table2 test after the GitHub Actions trigger issue
was diagnosed.

This script:
1. Downloads SH0ES compact ladder files:
   - allc covariance
   - alll design matrix
   - ally observation vector
   - table2.tex Cepheid table

2. Solves the compact SH0ES ladder:
   y = X beta
   X = alll.T
   C = allc

3. Recovers the 38/41/43 Cepheid spine rows.

4. Maps compact spine rows onto parsed Table2 Cepheid rows.

5. Summarizes residual pressure by:
   - host guess
   - compact signature cluster
   - Table2 numeric fields
   - host/table candidate vectors after projection out of the original 47-parameter model

Boundary:
This is not proof of TAIRID.
This is not H0 resolution.
This is not BAO, Planck, or a full cosmology model.
This is a host/table residual-pressure audit after the original SH0ES compact ladder fit.
"""
...
OUTDIR = Path("tairid_table2_host_residual_v1_2_outputs")
...
def fetch_url(url):
    req = urllib.request.Request(
        url,
        headers={
            "User-Agent": "TAIRID-Table2-host-residual-v1-2",
            "Accept": "*/*",
        },
    )
...
def parse_table2_tex(path):
    ...
    write_csv(OUTDIR / "table2_all_rows_v1_2.csv", all_rows)
    write_csv(OUTDIR / "table2_data_rows_v1_2.csv", data_rows)
    write_csv(OUTDIR / "table2_header_rows_v1_2.csv", header_rows)
...
def map_table2_to_spine(row_rows, table2_data_rows):
    ...
    # Some parses include extra header-like rows. If table rows are longer,
    # keep the last rows to align with prior observed 3130 Cepheid rows.
    ...
    write_csv(OUTDIR / "table2_compact_host_mapped_rows_v1_2.csv", mapped)
...
def summarize_hosts(mapped_rows):
    ...
    write_csv(OUTDIR / "table2_host_summary_v1_2.csv", host_rows)

    return host_rows


def summarize_cluster_hosts(mapped_rows):
    grouped = defaultdict(list)
    ...
    for cluster_id, rows in grouped.items():
        ...
        out.append(
            {
                "signature_cluster_id": int(cluster_id),
                "row_count": len(rows),
                ...
                "dominant_host_count": int(dominant_host_count),
    ...
    out = sorted(
        out,
        key=lambda r: (-r["mean_abs_residual"], -r["row_count"], r["signature_cluster_id"]),
    )

    write_csv(OUTDIR / "table2_cluster_host_summary_v1_2.csv", out)
...
def numeric_feature_correlations(mapped_rows):
    ...
    write_csv(OUTDIR / "table2_numeric_correlations_v1_2.csv", out)
...
def build_audit_vectors(mapped_rows, host_summary, cluster_summary, numeric_corr, y_length):
    vectors = []
    ...
    def add(name, values, kind):
        ...
        if len(values) != y_length:
            raise ValueError(f"{name}: vector length {len(values)} != expected {y_length}")
    ...
    # Union of top pressure hosts.
    top_hosts = {h["host_guess"] for h in host_summary[:10] if h["row_count"] >= 5}
    host_union = np.zeros(y_length, dtype=float)

    for row in mapped_rows:
        if row["host_guess"] in top_hosts:
            host_union[int(row["compact_observation_index"])] = 1.0

    add("host_union_top10_pressure", host_union, "host_union")
    ...
    for cluster in cluster_summary[:20]:
        ...
        add(
            f"cluster_{cluster['signature_cluster_id']}_host_{safe_name(cluster['dominant_host'])}_n{cluster['row_count']}",
            mask,
            "cluster_host_mask",
        )

    # Numeric features and high/low masks.
    for item in numeric_corr[:12]:
        column = item["numeric_column"]
        values = np.zeros(y_length, dtype=float)
        observed_values = []

        for row in mapped_rows:
            try:
                numeric_value = float(row.get(column, ""))
            except Exception:
                continue
        ...
        fill = float(np.nanmean(observed_values))

        for row in mapped_rows:
            idx = int(row["compact_observation_index"])

            try:
                numeric_value = float(row.get(column, fill))
            except Exception:
                numeric_value = fill

            values[idx] = numeric_value if np.isfinite(numeric_value) else fill

        add(f"numeric_{column}", values, "table2_numeric_feature")
        ...
        high_threshold = np.percentile(mapped_values, 90)
        low_threshold = np.percentile(mapped_values, 10)
        ...
        add(f"numeric_{column}_top10", high_mask, "table2_numeric_extreme")
        add(f"numeric_{column}_bottom10", low_mask, "table2_numeric_extreme")

    # Table row-order control.
...
def random_control_audit(y, X, c_factor, baseline, count, label):
    ...
    summary = {
        ...
        **stats(same, "same_count"),
        **stats(block, "contiguous_block"),
    }
...
def code_context_search(aux_results):
    ...
    write_csv(OUTDIR / "table2_host_context_hits_v1_2.csv", rows)

    return rows


def make_plots(mapped_rows, host_summary, cluster_summary, numeric_corr, audit_rows):
    if not mapped_rows:
        return
    ...
    plt.title("Table2 mapped compact residuals v1.2")
    plt.tight_layout()
    plt.savefig(OUTDIR / "table2_mapped_residual_by_row_v1_2.png", dpi=160)
    plt.close()

    top_hosts = host_summary[:25]

    plt.figure(figsize=(13, 6))
    plt.bar(np.arange(len(top_hosts)), [h["mean_abs_residual"] for h in top_hosts])
    plt.xticks(np.arange(len(top_hosts)), [h["host_guess"] for h in top_hosts], rotation=45, ha="right")
    plt.ylabel("mean absolute residual")
    plt.title("Highest-pressure Table2 hosts v1.2")
    plt.tight_layout()
    plt.savefig(OUTDIR / "table2_host_pressure_v1_2.png", dpi=160)
    plt.close()

    top_clusters = cluster_summary[:30]

    plt.figure(figsize=(14, 6))
    plt.bar(np.arange(len(top_clusters)), [c["mean_abs_residual"] for c in top_clusters])
    plt.xticks(
        np.arange(len(top_clusters)),
        [f"{c['signature_cluster_id']}:{c['dominant_host']}" for c in top_clusters],
        rotation=60,
        ha="right",
        fontsize=8,
    )
    plt.ylabel("mean absolute residual")
    plt.title("High-pressure compact clusters aligned to Table2 hosts v1.2")
    plt.tight_layout()
    plt.savefig(OUTDIR / "table2_cluster_host_pressure_v1_2.png", dpi=160)
    plt.close()

    if numeric_corr:
        top_numeric = numeric_corr[:20]

        plt.figure(figsize=(12, 6))
        plt.bar(np.arange(len(top_numeric)), [abs(n["corr_with_abs_residual"]) for n in top_numeric])
        plt.xticks(np.arange(len(top_numeric)), [n["numeric_column"] for n in top_numeric], rotation=45, ha="right")
        plt.ylabel("|corr with abs residual|")
        plt.title("Table2 numeric feature correlations v1.2")
        plt.tight_layout()
        plt.savefig(OUTDIR / "table2_numeric_correlations_v1_2.png", dpi=160)
        plt.close()
    ...
    if plot_rows:
        ...
        plt.title("Host/Table2 candidate residual alignment v1.2")
        plt.tight_layout()
        plt.savefig(OUTDIR / "table2_candidate_delta_chi2_v1_2.png", dpi=160)
        plt.close()
        ...
        plt.title("Host/Table2 candidate survival after SH0ES projection v1.2")
        plt.tight_layout()
        plt.savefig(OUTDIR / "table2_candidate_nondegenerate_v1_2.png", dpi=160)
        plt.close()
...
def decide_status(map_status, audit_rows, host_summary, numeric_corr, random_summaries):
    ...
    host_rows = [r for r in ok_rows if r.get("candidate_kind") in ["host_mask", "host_union"]]
    cluster_rows = [r for r in ok_rows if r.get("candidate_kind") == "cluster_host_mask"]
    numeric_rows = [r for r in ok_rows if r.get("candidate_kind") in ["table2_numeric_feature", "table2_numeric_extreme"]]

    best_host = max(host_rows, key=lambda r: r["delta_chi2_score"]) if host_rows else None
    best_cluster = max(cluster_rows, key=lambda r: r["delta_chi2_score"]) if cluster_rows else None
    best_numeric = max(numeric_rows, key=lambda r: r["delta_chi2_score"]) if numeric_rows else None
    ...
    if not map_status.get("exact_count_match"):
        return (
            "table2_host_map_count_mismatch_but_audited",
            6,
            "Table2 and compact spine rows did not match exactly; inspect parser before relying on host labels.",
            best_cases,
        )
...
def main():
    print("")
    print("TAIRID Table2 host residual audit v1.2 starting.")
    ...
    write_csv(OUTDIR / "download_ledger_v1_2.csv", ledger)
    write_json(OUTDIR / "download_attempts_v1_2.json", {"compact": downloads, "auxiliary": aux_results})
    ...
    write_json(OUTDIR / "parse_meta_v1_2.json", parse_meta)
    write_json(OUTDIR / "parse_errors_v1_2.json", parse_errors)
    ...
    if parse_errors or not all(key in parsed for key in ["allc", "alll", "ally"]) or not table2_result:
        summary = {
            "test_name": "TAIRID Table2 host residual audit v1.2",
            "boundary": "Download/parse failure. No host-map result.",
            "final_status": "table2_host_residual_v1_2_parse_or_download_failed",
        ...
        write_json(OUTDIR / "table2_host_residual_v1_2_summary.json", summary)
    ...
    write_csv(OUTDIR / "compact_row_map_v1_2.csv", row_rows)
    write_csv(OUTDIR / "compact_cluster_map_v1_2.csv", cluster_rows)

    mapped_rows, map_status = map_table2_to_spine(row_rows, table2_data_rows)

    host_summary = summarize_hosts(mapped_rows)
    cluster_host_summary = summarize_cluster_hosts(mapped_rows)
    numeric_corr = numeric_feature_correlations(mapped_rows)

    vectors = build_audit_vectors(
        mapped_rows,
        host_summary,
        cluster_host_summary,
        numeric_corr,
        len(y),
    )
    ...
    write_csv(OUTDIR / "candidate_audit_v1_2.csv", audit_rows)

    random_summaries = []
    random_detail_rows = []
    seen_counts = set()

    top_control_candidates = [
        r for r in audit_rows
        if r.get("status") == "ok"
        and r.get("candidate_kind") != "control"
        and 0 < int(r.get("count_nonzero_raw", 0)) < len(y)
    ][:3]

    for candidate in top_control_candidates:
    ...
    write_json(OUTDIR / "random_control_summaries_v1_2.json", random_summaries)
    write_csv(OUTDIR / "random_control_details_v1_2.csv", random_detail_rows)
    ...
    make_plots(mapped_rows, host_summary, cluster_host_summary, numeric_corr, audit_rows)
    ...
    summary = {
        ...
        "candidate_audit": audit_rows[:100],
        "random_control_summaries": random_summaries,
        "best_cases": best_cases,
        "code_context_hits_count": len(code_hits),
        "output_files": {
            "summary_json": str(OUTDIR / "table2_host_residual_v1_2_summary.json"),
            "summary_txt": str(OUTDIR / "table2_host_residual_v1_2_summary.txt"),
            "mapped_rows_csv": str(OUTDIR / "table2_compact_host_mapped_rows_v1_2.csv"),
            "host_summary_csv": str(OUTDIR / "table2_host_summary_v1_2.csv"),
            "cluster_host_summary_csv": str(OUTDIR / "table2_cluster_host_summary_v1_2.csv"),
            "numeric_correlations_csv": str(OUTDIR / "table2_numeric_correlations_v1_2.csv"),
            "candidate_audit_csv": str(OUTDIR / "candidate_audit_v1_2.csv"),
            "random_control_summaries_json": str(OUTDIR / "random_control_summaries_v1_2.json"),
            "random_control_details_csv": str(OUTDIR / "random_control_details_v1_2.csv"),
            "context_hits_csv": str(OUTDIR / "table2_host_context_hits_v1_2.csv"),
            "plots": [
                str(OUTDIR / "table2_mapped_residual_by_row_v1_2.png"),
                str(OUTDIR / "table2_host_pressure_v1_2.png"),
                str(OUTDIR / "table2_cluster_host_pressure_v1_2.png"),
                str(OUTDIR / "table2_numeric_correlations_v1_2.png"),
                str(OUTDIR / "table2_candidate_delta_chi2_v1_2.png"),
                str(OUTDIR / "table2_candidate_nondegenerate_v1_2.png"),
            ],
        },
        "interpretation": {
            "success_condition": (
                "Compact 38/41/43 spine rows align with Table2 Cepheid rows and at least one "
                "host/table direction survives projection enough to justify stricter validation."
            ),
            "failure_condition": (
                "The mapping works, but host/table directions are absorbed or not residual-aligned "
                "strongly enough for a new model."
            ),
            "truth_boundary": (
                "This does not prove TAIRID. It only audits Cepheid/Table2 host-mapped residual "
                "pressure after the SH0ES compact ladder fit."
            ),
        },
    }
    ...
    with open(OUTDIR / "table2_host_residual_v1_2_summary.txt", "w", encoding="utf-8") as f:
        f.write("TAIRID Table2 host residual audit v1.2\n\n")
    ...
    print("")
    print("TAIRID Table2 host residual audit v1.2 complete.")
    print("Created:")
    print("  tairid_table2_host_residual_v1_2_outputs/table2_host_residual_v1_2_summary.json")
    print("  tairid_table2_host_residual_v1_2_outputs/table2_host_residual_v1_2_summary.txt")
    print("  tairid_table2_host_residual_v1_2_outputs/table2_compact_host_mapped_rows_v1_2.csv")
    print("  tairid_table2_host_residual_v1_2_outputs/table2_host_summary_v1_2.csv")
    print("")
