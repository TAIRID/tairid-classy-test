#!/usr/bin/env python3
"""
TAIRID Planck low-l + lensing evaluate probe.

Purpose:
Try to evaluate fixed TAIRID proxy candidates against installed Cobaya Planck
likelihood components:

- planck_2018_lowl.TT
- planck_2018_lowl.EE
- planck_2018_lensing.clik

Boundary:
This is not the full Planck likelihood.
This does not include high-l Plik TTTEEE.
This does not include full nuisance-parameter exploration.
This does not prove TAIRID cosmology.
This is a first likelihood-evaluation bridge test.
"""

import json
import math
import platform
import subprocess
import sys
import traceback
from pathlib import Path

import yaml


OUTDIR = Path("planck_low_l_lensing_evaluate_outputs")
OUTDIR.mkdir(parents=True, exist_ok=True)

RUN_DIR = OUTDIR / "candidate_yamls"
RUN_DIR.mkdir(parents=True, exist_ok=True)

PACKAGES_DIR = Path("cobaya_planck_packages_runtime")
PACKAGES_DIR.mkdir(parents=True, exist_ok=True)

H = 0.66893180
H0 = H * 100.0
OMEGA_NEUTRAL_PHYSICAL = 0.1200000000

BASE_CONSTANTS = {
    "H0": H0,
    "omega_b": 0.0223700000,
    "n_s": 0.9649,
    "A_s": 2.100549e-9,
    "tau_reio": 0.0544,
}

COMMON_CLASS_EXTRA = {
    "output": "tCl,pCl,lCl,mPk",
    "lensing": "yes",
    "l_max_scalars": 2500,
    "P_k_max_1/Mpc": 50.0,
    "z_max_pk": 3.0,
    "z_pk": "0",
    "N_ur": 3.046,
    "Omega_k": 0.0,
    "Omega_Lambda": 0.6817397872,
    "YHe": 0.245,
    "T_cmb": 2.7255,
}

CASES = [
    {
        "name": "cdm_baseline",
        "label": "CDM baseline",
        "warm_fraction": 0.0,
        "m_ncdm_eV": None,
    },
    {
        "name": "strict_anchor_1p5pct_12p5eV",
        "label": "Strict safety anchor",
        "warm_fraction": 0.015,
        "m_ncdm_eV": 12.5,
    },
    {
        "name": "best_score_1p75pct_15eV",
        "label": "Best matrix score anchor",
        "warm_fraction": 0.0175,
        "m_ncdm_eV": 15.0,
    },
    {
        "name": "stronger_s8_2pct_15eV",
        "label": "Stronger S8 but lensing warning",
        "warm_fraction": 0.020,
        "m_ncdm_eV": 15.0,
    },
    {
        "name": "warning_old_5pct_20eV",
        "label": "Old S8-helpful lensing-warning case",
        "warm_fraction": 0.050,
        "m_ncdm_eV": 20.0,
    },
]

REPORT = {
    "boundary": "First Planck low-l + lensing evaluate probe only. Not full Planck and not proof of TAIRID cosmology.",
    "python": {
        "version": sys.version,
        "platform": platform.platform(),
        "executable": sys.executable,
    },
    "packages_dir": str(PACKAGES_DIR),
    "install_commands": [],
    "cases": [],
}


def run_command(label, command, timeout=1800):
    print("")
    print("Running:", label)
    print("Command:", " ".join(command))

    entry = {
        "label": label,
        "command": command,
        "returncode": None,
        "status": "not_run",
        "stdout_tail": "",
        "stderr_tail": "",
    }

    try:
        proc = subprocess.run(
            command,
            stdout=subprocess.PIPE,
            stderr=subprocess.PIPE,
            text=True,
            timeout=timeout,
            check=False,
        )

        entry["returncode"] = proc.returncode
        entry["status"] = "passed" if proc.returncode == 0 else "nonzero_exit"
        entry["stdout_tail"] = proc.stdout[-8000:]
        entry["stderr_tail"] = proc.stderr[-8000:]

        print("Return code:", proc.returncode)
        print("STDOUT tail:")
        print(entry["stdout_tail"])
        print("STDERR tail:")
        print(entry["stderr_tail"])

    except Exception as exc:
        entry["status"] = "failed_exception"
        entry["error"] = str(exc)
        entry["traceback"] = traceback.format_exc()
        print("FAILED:", exc)

    return entry


def install_planck_components():
    components = [
        "planck_2018_lowl.TT",
        "planck_2018_lowl.EE",
        "planck_2018_lensing.clik",
    ]

    for component in components:
        entry = run_command(
            "install_" + component.replace(".", "_"),
            ["cobaya-install", component, "-p", str(PACKAGES_DIR)],
            timeout=1800,
        )
        REPORT["install_commands"].append(entry)


def make_candidate_info(case):
    warm_fraction = float(case["warm_fraction"])
    m_ncdm_eV = case["m_ncdm_eV"]

    omega_cdm = OMEGA_NEUTRAL_PHYSICAL
    extra_args = dict(COMMON_CLASS_EXTRA)

    if warm_fraction > 0.0:
        omega_warm = OMEGA_NEUTRAL_PHYSICAL * warm_fraction
        omega_cold = OMEGA_NEUTRAL_PHYSICAL * (1.0 - warm_fraction)

        omega_cdm = omega_cold

        extra_args["N_ncdm"] = 1
        extra_args["m_ncdm"] = float(m_ncdm_eV)
        extra_args["Omega_ncdm"] = omega_warm / (H * H)

    params = dict(BASE_CONSTANTS)
    params["omega_cdm"] = omega_cdm

    info = {
        "debug": False,
        "packages_path": str(PACKAGES_DIR),
        "theory": {
            "classy": {
                "extra_args": extra_args,
            }
        },
        "likelihood": {
            "planck_2018_lowl.TT": None,
            "planck_2018_lowl.EE": None,
            "planck_2018_lensing.clik": None,
        },
        "params": params,
        "sampler": {
            "evaluate": None,
        },
        "output": str(OUTDIR / "cobaya_runs" / case["name"]),
    }

    return info


def write_yaml(case, info):
    path = RUN_DIR / f"{case['name']}.yml"
    with open(path, "w") as f:
        yaml.safe_dump(info, f, sort_keys=False)
    return path


def evaluate_candidate(case):
    info = make_candidate_info(case)
    yaml_path = write_yaml(case, info)

    entry = run_command(
        "evaluate_" + case["name"],
        [sys.executable, "-m", "cobaya", "run", str(yaml_path), "--force"],
        timeout=1800,
    )

    result = {
        "name": case["name"],
        "label": case["label"],
        "warm_fraction": case["warm_fraction"],
        "m_ncdm_eV": case["m_ncdm_eV"],
        "yaml_file": str(yaml_path),
        "status": entry["status"],
        "returncode": entry["returncode"],
        "stdout_tail": entry["stdout_tail"],
        "stderr_tail": entry["stderr_tail"],
    }

    combined_text = (entry.get("stdout_tail") or "") + "\n" + (entry.get("stderr_tail") or "")

    likelihood_lines = []
    for line in combined_text.splitlines():
        low = line.lower()
        if "logpost" in low or "loglike" in low or "chi2" in low or "minuslogpost" in low:
            likelihood_lines.append(line)

    result["likelihood_related_lines"] = likelihood_lines[-50:]

    return result


def main():
    try:
        import cobaya
        REPORT["cobaya_version"] = getattr(cobaya, "__version__", "unknown")
    except Exception as exc:
        REPORT["cobaya_import_error"] = str(exc)

    install_planck_components()

    for case in CASES:
        print("")
        print("Evaluating candidate:", case["name"])

        try:
            result = evaluate_candidate(case)
        except Exception as exc:
            result = {
                "name": case["name"],
                "label": case["label"],
                "warm_fraction": case["warm_fraction"],
                "m_ncdm_eV": case["m_ncdm_eV"],
                "status": "failed_exception",
                "error": str(exc),
                "traceback": traceback.format_exc(),
            }

        REPORT["cases"].append(result)

    passed_cases = [case for case in REPORT["cases"] if case.get("returncode") == 0]

    if passed_cases:
        REPORT["interpretation"] = (
            "At least one fixed candidate evaluated successfully against the low-l + lensing Planck components. "
            "Inspect likelihood-related lines before interpreting any score."
        )
    else:
        REPORT["interpretation"] = (
            "No candidate completed evaluation successfully. This is a bridge failure, not a cosmology failure. "
            "Inspect stderr tails to identify parameter, nuisance, or likelihood-path issues."
        )

    report_path = OUTDIR / "planck_low_l_lensing_evaluate_report.json"
    report_path.write_text(json.dumps(REPORT, indent=2))

    with open(OUTDIR / "planck_low_l_lensing_evaluate_summary.txt", "w") as f:
        f.write("TAIRID Planck low-l + lensing evaluate probe\n")
        f.write("\n")
        f.write("Boundary: not full Planck, not proof of TAIRID cosmology.\n")
        f.write("\n")
        f.write("Install commands:\n")
        for cmd in REPORT["install_commands"]:
            f.write(f"{cmd['label']}: {cmd['status']} returncode={cmd['returncode']}\n")

        f.write("\n")
        f.write("Candidate evaluate results:\n")
        for case in REPORT["cases"]:
            f.write(f"{case['name']}: {case['status']} returncode={case.get('returncode')}\n")

        f.write("\n")
        f.write("Interpretation:\n")
        f.write(REPORT["interpretation"] + "\n")

    print("")
    print("TAIRID Planck low-l + lensing evaluate probe complete.")
    print("Created:")
    print("  planck_low_l_lensing_evaluate_outputs/planck_low_l_lensing_evaluate_report.json")
    print("  planck_low_l_lensing_evaluate_outputs/planck_low_l_lensing_evaluate_summary.txt")
    print("")
    print("Boundary:")
    print("  This is not full Planck.")
    print("  This is the first likelihood-evaluation bridge test.")


if __name__ == "__main__":
    main()
