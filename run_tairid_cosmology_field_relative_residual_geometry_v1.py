#!/usr/bin/env python3
"""
TAIRID cosmology field-relative residual geometry v1.
...
A positive result means:
    residual geometry survives offset freedom and deserves a sharper multi-observable test.

A negative result means:
    the supernova residual lane does not yet carry TAIRID field-relative structure
    beyond offset/calibration freedom.
...
OUTDIR = Path("tairid_cosmology_field_relative_residual_geometry_v1_outputs")
...
def gls_fit(y, c_factor, X, names):
    ...
    param_errors = {f"{name}_err": float(math.sqrt(max(A_inv[i, i], 0.0))) for i, name in enumerate(names)}
    return {
        "params": params,
...
def fit_baseline_and_shape(y, c_factor, base_X, base_names, shape_cols, shape_names):
    ...
    p = float(chi2.sf(max(delta, 0.0), dof_delta))

    return base, shaped, {
        "delta_chi2_improvement": delta,
        "delta_dof": dof_delta,
        "p_value_chi2_improvement": p,
    }
...
def permutation_geometry(z, residual, sigma, bins, observed_metrics):
    ...
    return {
        ...
        "perm_quad_abs_95": float(np.percentile(np.abs([m["quad_coeff_x2"] for m in metrics if np.isfinite(m["quad_coeff_x2"])]), 95)),
        "perm_lag1_abs_95": float(np.percentile(np.abs([m["lag1_autocorr"] for m in metrics if np.isfinite(m["lag1_autocorr"])]), 95)),
    }
...
def plot_frozen_shape(z, title, path):
    ...
    plt.xlabel("z")
    plt.ylabel("G(z)")
    plt.title(title + " — frozen gate")
    plt.legend(fontsize=8)
    plt.tight_layout()
    plt.savefig(path.with_name(path.stem + "_gate.png"), dpi=160)
    plt.close()
    ...
    plt.title(title + " — frozen residual shape")
    plt.legend(fontsize=8)
    plt.tight_layout()
    plt.savefig(path.with_name(path.stem + "_mu_shape.png"), dpi=160)
    plt.close()
...
def decide_status(rows):
    ...
    best_perm = min(
        ...
        key=lambda r: min(
            [
                p for p in [
                    r.get("perm_p_abs_quad_coeff_x2"),
                    r.get("perm_p_abs_lag1_autocorr"),
                    r.get("perm_p_breach_fraction"),
                ]
                if p is not None
            ] or [1.0]
        )
    ...
    perm_ps = [
        p for p in [
            best_perm.get("perm_p_abs_quad_coeff_x2"),
            best_perm.get("perm_p_abs_lag1_autocorr"),
            best_perm.get("perm_p_breach_fraction"),
        ]
        if p is not None
    ]
    perm_pass = bool(perm_ps and min(perm_ps) <= 0.01)
...
def main():
    print("")
    print("TAIRID cosmology field-relative residual geometry v1 starting.")
    ...
    all_rows = []
    all_bin_rows_path = OUTDIR / "cosmology_field_relative_binned_residuals.csv"
    ...
    for scheme in OFFSET_SCHEMES:
        ...
        residual_plot = OUTDIR / f"cosmology_field_relative_residuals_{scheme}.png"
        plot_residuals(
            z,
            baseline["residual"],
            bin_rows,
    ...
    plot_frozen_shape(z, "TAIRID cosmology field-relative residual geometry v1", OUTDIR / "cosmology_field_relative_frozen_shape.png")
    plots.append(str(OUTDIR / "cosmology_field_relative_frozen_shape_gate.png"))
    plots.append(str(OUTDIR / "cosmology_field_relative_frozen_shape_mu_shape.png"))

    final_status, readiness_score, next_wall, best_cases = decide_status(all_rows)

    summary = {
        "test_name": "TAIRID cosmology field-relative residual geometry v1",
        "boundary": (
        ...
        "output_files": {
            "summary_json": str(OUTDIR / "cosmology_field_relative_residual_geometry_v1_summary.json"),
            "summary_txt": str(OUTDIR / "cosmology_field_relative_residual_geometry_v1_summary.txt"),
            "scheme_results_csv": str(OUTDIR / "cosmology_field_relative_scheme_results.csv"),
    ...
    write_json(OUTDIR / "cosmology_field_relative_residual_geometry_v1_summary.json", summary)

    csv_path = OUTDIR / "cosmology_field_relative_scheme_results.csv"
            ...
            writer.writerow(
                {
                    ...
                    "baseline_offsets_json": json.dumps(r["baseline_offsets"]),
                    "frozen_shape_params_json": json.dumps(r["frozen_shape_params"]),
                    "generic_curvature_params_json": json.dumps(r["generic_curvature_params"]),
                }
            )

    with open(OUTDIR / "cosmology_field_relative_residual_geometry_v1_summary.txt", "w") as f:
        f.write("TAIRID cosmology field-relative residual geometry v1\n\n")
        ...
        f.write("Best cases:\n")
        f.write(json.dumps(best_cases, indent=2) + "\n\n")
    ...
    print("")
    print("TAIRID cosmology field-relative residual geometry v1 complete.")
    print("Created:")
    print("  tairid_cosmology_field_relative_residual_geometry_v1_outputs/cosmology_field_relative_residual_geometry_v1_summary.json")
    print("  tairid_cosmology_field_relative_residual_geometry_v1_outputs/cosmology_field_relative_residual_geometry_v1_summary.txt")
    print("  tairid_cosmology_field_relative_residual_geometry_v1_outputs/cosmology_field_relative_scheme_results.csv")
    print("  tairid_cosmology_field_relative_residual_geometry_v1_outputs/cosmology_field_relative_binned_residuals.csv")
    print("")
    print("Boundary:")
